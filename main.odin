...
main :: proc()
{
	...
	init_vulkan(&ctx);
	font := build_font(&ctx, "res/Inconsolata-Regular.ttf", 12);
	
	tex := create_texture(&ctx, "res/brick_edg32_variants.png");
	//upload_texture(&ctx, tex, 0);
	...
	for !glfw.WindowShouldClose(window.handle)
	{
		...
		buffer_draw(&ctx, &test_buffer, {0, 0});
